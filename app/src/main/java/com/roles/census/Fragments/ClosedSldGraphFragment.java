...
public class ClosedSldGraphFragment extends Fragment {
    PieChart pieChart;
    int count=10;
    String email,password,username,isEdit,Closed_Sealed;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        closelist= (ArrayList<HashMap<String, String>>) args.getSerializable("closelist");
        email= args.getString("email");
        password= args.getString("password");
        username= args.getString("username");
        isEdit= args.getString("isEdit");
        Closed_Sealed = args.getString("Closed_Sealed");
        // array of images
        text.setText("Close Sealed ("+Closed_Sealed+")");
  ...
  /*      for (int i=0;i<funclist.size();i++) {
        ...
        int c=ContextCompat.getColor(getContext(), R.color.lightblue);
        ...
        for (int i=0;i<closelist.size();i++) {
            entries.add(new Entry(Integer.parseInt(closelist.get(i).get("Total")),i));
        }
        ...
        for (int i=0;i<closelist.size();i++) {
            xVals.add(closelist.get(i).get("TypeDesc")+" ("+closelist.get(i).get("Total")+")");
        }
        //xVals.add("May");
        //xVals.add("June");
